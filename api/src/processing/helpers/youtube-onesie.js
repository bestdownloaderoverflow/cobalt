...
// I'm planning to rewrite it myself 
...
export const getBasicInfo = async (yt, videoId, client) => {
	// todo: this context readjusting here is stupid
	// & incomplete
	const ctx = structuredClone(yt.session.context);
	ctx.client.clientName = Constants.CLIENTS[client].NAME;
	ctx.client.clientVersion = Constants.CLIENTS[client].VERSION;
	ctx.thirdParty = { embedUrl: "https://www.google.com/" };
	...
	const initResponse = await yt.session.http.fetch_function(redirectorResponse.headers.get("location"), {
		method: "POST",
		headers: {
			"accept": "*/*",
